...
@tree.command(name="find", description="Do a deep dive on a specific token")
@app_commands.describe(symbol="Token symbol, e.g., sol")
async def find_command(interaction: discord.Interaction, symbol: str):
    base_url = "https://api.geckoterminal.com/api/v2/tokens/info_recently_updated"
    data = requests.get(base_url).json().get("data", [])
    for token in data:
        attr = token["attributes"]
...
@tree.command(name="matrix", description="List the 10 newest tokens with GT score and risk")
async def matrix_command(interaction: discord.Interaction):

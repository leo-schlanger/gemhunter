...
class GemHunter(discord.app_commands.Group):
    def __init__(self):
    ...
    async def react_command(self, interaction: discord.Interaction, symbol: str):
    ...
    @app_commands.describe(symbol="Token symbol, e.g., sol")
    async def find_command(self, interaction: discord.Interaction, symbol: str):
        base_url = "https://api.geckoterminal.com/api/v2/tokens/info_recently_updated"
        data = requests.get(base_url).json().get("data", [])
        for token in data:
            attr = token["attributes"]
    ...
    async def matrix_command(self, interaction: discord.Interaction):
